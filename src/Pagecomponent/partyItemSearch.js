...
class PartyItemSearch extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      price: "",
  ...
  assign_global_values_to_Search_field = () => {
    let price = "";
    let local_price = Global.partyitem_search_values_obj.price.split(",");
    if (local_price == "") {
      price = "Select price";
    } else {
    ...
    this.setState({
      price_show: price,
      order: Global.partyitem_search_values_obj.order,
    });
  };
  ...
  order(e) {
    Global.partyitem_search_values_obj.order = e.target.value;
...
export default PartyItemSearch;
