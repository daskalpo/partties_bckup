...
class PartyItemDetailsComponent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cake_message: "",
      partyitem_quantity_input: 1,
      wishlist: "",
      delivery_pincode: "",
    };
  }

  refreshCakeSize = (completePartyItemDetails, e) => {
    this.props.refreshCake(e.target.value, completePartyItemDetails);
  };
  ...
  customised_nevigate = (nav) => {
    ...
    this.props.history.push(
      "/customize-cake/" + this.props.completePartyItemDetails.slug + ""
    );
  };
  ...
  addToCart = (completePartyItemDetails, purpose) => {
    console.log(purpose);

    let local_partyitem_quantity = this.state.partyitem_quantity_input;

    if (local_partyitem_quantity == "") {
      Helper.alert("Please choose Quantity");
      return false;
    }
    if (parseInt(local_partyitem_quantity) === 0) {
      Helper.alert(" Quantity cannot be Zero");
      return false;
    }

    console.log(local_partyitem_quantity);

    let user = window.localStorage.getItem("LOGGEDUSER");
    let local_completePartyItemDetails = completePartyItemDetails;

    local_completePartyItemDetails["price"] = this.props.clickedProductPrice;
    local_completePartyItemDetails[
      "product_variation_id"
    ] = this.props.active_variation_id;
    local_completePartyItemDetails["weight"] = this.props.active_weight;
    local_completePartyItemDetails["message"] = this.state.cake_message;
    local_completePartyItemDetails["quantitys"] = local_partyitem_quantity;

    Helper.cart_json_maker(
      local_completePartyItemDetails,
      "product_slug_api"
    ).then((partyitem) => {
      if (user == "" || user === null || user === undefined) {
        Helper.localstorage___addToCart(partyitem);
        Helper.assign_cart_item_modify(local_partyitem_quantity);
      ...
      if (user != "" && user !== null && user !== undefined) {
        ...
        Helper.add_directly_to_database_on_login(partyitem).then((status) => {
          console.log(status);
          if (purpose != "no_nevigation") {
            this.nevigate("cart");
          } else {
  ...
  componentDidMount = () => {
    ...
    window.scrollTo(0, 0);
    Helper.loadScript();
    if (!this.props.completePartyItemDetails.wishlist) {
  ...
  add_to_wishlist = (completePartyItemDetails) => {
    let user = window.localStorage.getItem("LOGGEDUSER");
    if (user == "" || user === null || user === undefined) {
      document.getElementById("loginss").click();
    }
  ...
  partyitem_quantity_input(e) {
    if (e.target.value.length < 4) {
      this.setState({ partyitem_quantity_input: e.target.value });
    }
  }
  ...
  render() {
    ...
    if (this.props.completePartyItemDetails.reviews_count[0] !== undefined) {
      aggregate = this.props.completePartyItemDetails.reviews_count[0]
        .aggregate;
      total_reviews = this.props.completePartyItemDetails.reviews_count[0]
        .aggregate;
      rating =
        this.props.completePartyItemDetails.reviews_count[0].ratingsum /
        total_reviews;
    ...
    var ProductMultipleImage = this.props.completePartyItemDetails.images.map(
      function (img, index) {
        return (
            ...
            <div className="row">
              <div className="col-md-12 breadcrumbWrapper">
                <ul className="breadcrumb">
                  ...
                  <li onClick={this.nevigate.bind(this, "party-items")}>
                    <Link to="#">Party Items</Link>
                  </li>
                  <li>{this.props.completePartyItemDetails.name}</li>
                </ul>
              </div>
            </div>
          </div>
                  ...
                  <div className="row">
                    ...
                    <div className="col-md-6">
                      <div className="products_name_detls_content">
                        <h2>{this.props.completePartyItemDetails.name}</h2>
                        ...
                        <ul>
                          <p className="custom_seller_name">
                            Seller:{" "}
                            {this.props.completePartyItemDetails.vendor.name}
                          </p>
                        </ul>
                        <ul className="product_pricings">
                          <li>
                            ...
                            <span className="price_main">
                              {this.props.clickedPartyItemPrice}
                            </span>
                          </li>
                        </ul>
                        <p className="sub_detls">
                          {this.props.completePartyItemDetails.description}
                        </p>
                        ...
                        <div className="customise_cake_detls margin_bottomcustomise_cake_detls">
                          <div className="form-group">
                            ...
                            <input
                              ...
                              onChange={this.partyitem_quantity_input.bind(
                                this
                              )}
                              value={this.state.partyitem_quantity_input}
                            />
                        ...
                        <ul className="btn_cart">
                          <li
                            onClick={this.addToCart.bind(
                              this,
                              this.props.completePartyItemDetails,
                              "no_nevigation"
                            )}
                      ...
                      <div className="customise_cake_detls margin_bottomcustomise_cake_detls">
                        <div className="form-group">
                          ...
                          <input
                            ...
                            className="form-control"
                            placeholder="Enter Quantity"
                            onChange={this.partyitem_quantity_input.bind(this)}
                            value={this.state.partyitem_quantity_input}
                          />
                          <datalist id="quantity">
                            <option value="1">1</option>
...
export default PartyItemDetailsComponent;
